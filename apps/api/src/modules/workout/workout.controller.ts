...
import type { CreateWorkoutPlanInput, ListWorkoutPlansQuery } from '@longeviq/shared';
...
export class WorkoutController {
  ...
  async createPlan(req: Request, res: Response): Promise<void> {
    const plan = await this.service.createPlan(
      req.user!.id,
      req.body as CreateWorkoutPlanInput,
      requestContext(req),
    );
  ...
  async listPlans(req: Request, res: Response): Promise<void> {
    const query = req.query as unknown as ListWorkoutPlansQuery;
